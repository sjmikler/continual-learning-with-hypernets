...
def get_training_function(model, loss_fn, optimizer, memory_regularization):
    """
    ...
    @tf.function
    def train_step(token, x, y, regularization_data):
        with tf.GradientTape() as tape:
            ...
            for old_token, inner_weights in regularization_data:
                _, weights = model([old_token, x])
                l2 = tf.reduce_sum(tf.square(weights - inner_weights))
                loss += memory_regularization * l2 / len(regularization_data)
    ...
    def train_epoch(token, steps_per_epoch, train_ds, test_ds=None,
        ...
        tbar = tqdm(islice(train_ds, steps_per_epoch),
                    ...
                    ascii=True)
        for x, y in tbar:
            outs = train_step(token, x, y, regularization)
